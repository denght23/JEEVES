...
RANK_THRESHOLD = 2048  # DC分界点

# 读取数据
with open('../data_4096_rack/workload_3k_1k_NO_PRI.json', 'r') as f:
    data = json.load(f)
...
# 1k卡数据64条流,其初始rank为3072
for num in range(1, 65):
    path = []
    current_name = f"F{num}a_DP0_PP0_TP0_Rank3072"
    end_pattern = f"B{num}a_DP0_PP0_TP0_Rank3072"
...
fig.update_layout(
    ...
    yaxis=dict(
        title="Stream Number",
        tickvals=list(range(1, 113)),# 48 + 64 + 1
        range=[0.5, 113.5],
        showgrid=True,
        gridcolor='lightgrey'
    ),
...
fig.write_html("timeline_4096_rack.html")
