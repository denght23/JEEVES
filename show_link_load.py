...
def plot_link_usage(link_usage, header):
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig(f"result/{header}/link_load.png")
...
def plot_link_util(link_usage, header):
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig(f"result/{header}/link_util.png")
...
delay = 2
file_name = f"link_num_{num}_bandwidth_{bandwidth}_delay_{delay}_route_"
...
util_file = f"result/{file_name}/link_util.txt"
...
odd_ratio = extract_real_optimal_ratio(load_file, flow_number_file, odd_links, bandwidth, num)
even_ratio = extract_real_optimal_ratio(load_file, flow_number_file, even_links, bandwidth, num)
plot_ratio(odd_ratio, f"{file_name}")
# plot_ratio(even_ratio, f"{file_name}/even")
...
plt.grid(True)
plt.legend()
plt.savefig(f"result/{file_name}/average_link_util.png")
